...
function setHoverSLRV() {
	tabslrv
		...
		.on({
			...
			TweenLite.to(logoSlrv, .7, { delay:.1, scale:1.2, x:40,y:-105, autoAlpha:1, ease:"easeOutExpo"});
...
function setHoverCO2W() {
	tabco2w
		...
		.on({
			...
			TweenLite.to(logoCo2w, .7, { delay:.1, y:-100, autoAlpha:1, ease:"easeOutExpo"});

			var desCo2w = $('#frame4').contents().find("#btCo2w");
			TweenLite.to(desCo2w, .7, { delay:.1, y:-100, autoAlpha:1, ease:"easeOutExpo"});
...
function setHoverKYRT() {
	tabkyrt
		...
		.on({
			...
			TweenLite.to(logoKyrt, .7, { delay:.1, x:300, y:-100, autoAlpha:1, ease:"easeOutExpo"});

			var desKyrt = $('#frame10').contents().find("#btKyrt");
			TweenLite.to(desKyrt, .7, { delay:.1, x:280, y:-100, autoAlpha:1, ease:"easeOutExpo"});
