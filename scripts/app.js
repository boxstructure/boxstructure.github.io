...
function setHoverCO2W() {
	tabco2w
		...
		.on({
			...
			TweenLite.to(logoCo2w, .5, { delay:.1, y:-100, autoAlpha:1, ease:"easeOutExpo"});

			var desCo2w = $('#frame4').contents().find("#btCo2w");
			TweenLite.to(desCo2w, .5, { delay:.1, y:-100, autoAlpha:1, ease:"easeOutExpo"});
...
function setHoverKYRT() {
	tabkyrt
		...
		.on({
			...
			TweenLite.to(logoKyrt, .5, { delay:.1, x:300, y:-100, autoAlpha:1, ease:"easeOutExpo"});

			var desKyrt = $('#frame10').contents().find("#btKyrt");
			TweenLite.to(desKyrt, .5, { delay:.1, x:280, y:-100, autoAlpha:1, ease:"easeOutExpo"});
